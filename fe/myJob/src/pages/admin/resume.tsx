...
const ResumePage = () => {
    ...
    const handleViewResume = (url: string | undefined) => {
        if (!url) {
            // Hiển thị thông báo lỗi nếu file k
            
            message.error("CV không tồn tại.");
            return false; // Ngăn không cho thực hiện hành động
        }
        return true; // Tiếp tục nếu file tồn tại
    };
    ...
    },
    {
        ...
        render: (_value, record) => (
            <a
                href={
                    record?.url
                        ? `${import.meta.env.VITE_BACKEND_URL}/storage/resume/${record?.url}`
                        : "#"
                }
                target={record?.url ? "_blank" : "_self"}
                onClick={(e) => {
                    if (!handleViewResume(record?.url)) {
                        e.preventDefault(); // Ngăn hành động mặc định nếu file không tồn tại
                    }
                }}
            >
                Chi tiết
